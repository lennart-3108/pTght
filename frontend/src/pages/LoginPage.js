...
function StatsPanel({ stats, statsLoading, statsError }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 12, padding: 12 }}>
      {items.map(item => (
        <div key={item.key} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, textAlign: 'center', padding: '12px 8px', backdropFilter: 'blur(6px)' }}>
          <StatNumber value={typeof item.val === 'number' ? item.val : 0} label={item.label} />
        </div>
      ))}
    </div>
  );
